...
class PackageCreator:
    def create(self, **kwargs):
        ...
        with tempfile.NamedTemporaryFile(prefix='singlem_package_creator',
                                         suffix='.fasta') as t:
                ...
                for t in tree_leaves:
                    if t not in found_sequence_names:
                        raise Exception("Found some sequences that were in the tree but not the"
                                        " unaligned sequences database e.g. %s. Something is likely"
                                        " amiss with the input GraftM package" % t)
