...
Skill corresponds to SingleM vSINGLEM_VERSION.
...
conda create -c conda-forge -c bioconda --override-channels \
  --name singlem 'singlem>=SINGLEM_VERSION'
...
```bash
docker pull wwood/singlem:SINGLEM_VERSION
# Run pipe directly:
docker run -v `pwd`:`pwd` wwood/singlem:SINGLEM_VERSION pipe \
  --sequences `pwd`/my.fastq.gz -p `pwd`/my.profile.csv --threads 4
```

### Singularity/Apptainer
```bash
singularity pull docker://wwood/singlem:SINGLEM_VERSION
singularity run -B `pwd`:`pwd` singlem_SINGLEM_VERSION.sif pipe \
  --sequences `pwd`/my.fastq.gz -p `pwd`/my.profile.csv --threads 4
```
