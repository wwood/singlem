...
Skill corresponds to SingleM v0.21.4.
...
# Run pipe directly:
docker run -v `pwd`:`pwd` wwood/singlem:0.21.4 pipe \
...
singularity run -B `pwd`:`pwd` singlem_0.21.4.sif pipe \
  --sequences `pwd`/my.fastq.gz -p `pwd`/my.profile.csv --threads 4
```
